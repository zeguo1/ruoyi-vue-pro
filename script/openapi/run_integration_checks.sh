...
mvn -q -pl yudao-server -am test \
  -Dtest=FullOpenApiExportTest,ContractSchemaCustomizerTest,WireContractTest,QueryContractRegressionTest,BackendHandoffContractTest,FullContractValidationTest,FullOpenApiModelAuditTest,ExternalOpenApiContractTest,OpenApiItemSchemaTest,ErpSaleOrderContractTest \
...
"$OPENAPI_PYTHON" script/openapi/verify_review_contract.py yudao-server/target/full-openapi-isolated.json \
  yudao-server/target/integration-review-verification.json
"$OPENAPI_PYTHON" script/openapi/audit_full_contract.py yudao-server/target/full-openapi-isolated.json \
  --output yudao-server/target/integration-schema-audit-raw.json \
  --inventory yudao-server/target/full-model-inventory.json
"$OPENAPI_PYTHON" script/openapi/verify_integration_contract.py \
  --document yudao-server/target/full-openapi-isolated.json \
  --inventory yudao-server/target/openapi-handler-inventory.json \
  --downloads script/openapi/v6-download-contracts.json \
  --examples yudao-server/target/v6-response-examples.json \
  --output yudao-server/target/integration-wire-verification-raw.json
