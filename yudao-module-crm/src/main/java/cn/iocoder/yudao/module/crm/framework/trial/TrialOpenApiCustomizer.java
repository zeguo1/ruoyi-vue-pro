...
public class TrialOpenApiCustomizer implements GlobalOpenApiCustomizer {
    ...
    @Override public void customise(OpenAPI document) {
        ...
        document.addExtension("x-mgs-trial-contract-version", "mgs-trial-v2-candidate");
        ...
        document.getComponents().addSecuritySchemes("MgsTrialSignature", new SecurityScheme().type(SecurityScheme.Type.APIKEY)
                ...
                .description("仅限 HTTPS 安全请求的 HMAC-SHA256 服务签名，不是静态 API Key。可信会话身份、MGS 短信验证凭据和签名头由服务端注入；新申请使用 mgs-trial-v2。"));
        document.getPaths().forEach((path, item) -> {
            ...
            item.readOperations().forEach(operation -> {
                ...
                operation.addExtension("x-mgs-service-contract", Map.of("version", "mgs-trial-v2", "capability", tool ? "TOOLS" : "EVENTS",
                        "transport", "https", "secureRequestRequired", true,
                        "modelMaySupplyIdentity", false, "trustedHeaders", List.of("X-Mgs-Trial-Version", "X-Mgs-Trial-Key", "X-Mgs-Trial-Timestamp", "X-Mgs-Trial-Nonce",
                                "X-Mgs-Trial-Subject", "X-Mgs-Trial-Verified", "X-Mgs-Trial-Verification", "X-Mgs-Trial-Confirmation", "X-Mgs-Trial-Idempotency", "X-Mgs-Trial-Signature")));
                if (tool) {
                    String action = path.substring(path.lastIndexOf('/') + 1);
                    operation.setOperationId(TOOLS.get(action));
                    if (action.equals("submit")) {
                        operation.addExtension("x-mgs-contact-verification", Map.of("issuer", "MGS", "method", "sms",
                                "proofHeader", "X-Mgs-Trial-Verification", "modelMaySupplyProof", false,
                                "verificationDoesNotConfirmOpening", true));
                    }
